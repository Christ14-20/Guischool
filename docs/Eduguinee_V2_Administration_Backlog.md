...
- Vérifié en désactivant temporairement la garde anti-downgrade pour confirmer qu'elle est bien testée (pas un test qui passerait de toute façon). Suite complète du projet : 651 tests passants (+19 nouveaux pour ce ticket), seuls les 2 tests `finance` pré-existants et sans lien ci-dessus échouent (confirmé identique sur `main` avant ce ticket).
