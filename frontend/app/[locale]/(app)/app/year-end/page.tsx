...
import { AlertCircle, CheckCircle2, Gavel, Loader2, Save, Users } from 'lucide-react';
...
export default function YearEndPage() {
  ...
  const currentYear = useMemo(() => years.find(y => String(y.id) === yearId), [years, yearId]);
    ...
    Promise.all([getSchoolYears(token), getClasses(token)])
      .then(([yearRes, classRes]) => {
        ...
        if (yearRes.results.length > 0) {
          const closing = yearRes.results.find(y => y.status === 'CLOTURE_EN_COURS');
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      .then(([ranking, existingDecisions]) => {
        ...
        const initialDecisions: Record<string, { decision: string; comment: string }> = {};
        ranking.forEach(s => {
          const existing = existingDecisions.results?.find((d: any) => d.eleve === s.eleve_id);
          if (existing) {
            initialDecisions[s.eleve_id] = { decision: existing.decision, comment: existing.commentaire || '' };
          } else {
            // Pre-fill based on threshold
            const autoDecision = (s.moyenne || 0) >= Number(threshold) ? 'ADMIS' : 'REDOUBLE';
            initialDecisions[s.eleve_id] = { decision: autoDecision, comment: '' };
          }
        });
  ...
  const handleDecisionChange = (studentId: string, value: string) => {
    setDecisions(prev => ({ ...prev, [studentId]: { ...prev[studentId], decision: value } }));
  };
    ...
    <div className="space-y-6">
      ...
      {!isClosing && yearId && (
        <Alert variant="destructive" className="bg-destructive/10">
          ...
          <AlertDescription>
            Les décisions de fin d'année ne sont activables que lorsque l'année scolaire est en statut 
        ...
        <Card className="md:col-span-1 h-fit">
          ...
          <CardContent className="space-y-4">
            <div className="space-y-2">
              <label className="text-xs font-medium uppercase text-muted-foreground">Année Scolaire</label>
              <Select value={yearId} onValueChange={(val) => setYearId(val ?? '')}>
                <SelectTrigger>
                  <SelectValue />
                </SelectTrigger>
                <SelectContent>
                  {years.map(y => <SelectItem key={y.id} value={String(y.id)}>{y.label} ({y.status})</SelectItem>)}
                </SelectContent>
              </Select>
            </div>
            ...
            <div className="space-y-2">
              ...
              <Select value={classId} onValueChange={(val) => setClassId(val ?? '')} disabled={!isClosing}>
                <SelectTrigger>
            ...
            <div className="space-y-2">
              <label className="text-xs font-medium uppercase text-muted-foreground">Seuil de passage</label>
              <Input type="number" value={threshold} onChange={e => setThreshold(e.target.value)} disabled={!isClosing} />
              <p className="text-[10px] text-muted-foreground italic">
                Les élèves avec une moyenne ≥ {threshold} seront marqués "ADMIS" par défaut.
              </p>
            </div>
          </CardContent>
        </Card>
              ...
              <Table>
                <TableHeader>
                  <TableRow>
                    ...
                    <TableHead className="text-center w-48">Décision Finale</TableHead>
                ...
                <TableBody>
                  {students.map((s) => {
                    ...
                    return (
                      <TableRow key={s.eleve_id} className={cn(!isAdmitted && "bg-destructive/5")}>
                        <TableCell>
                          <div className="font-medium">{s.nom} {s.prenom}</div>
                          <div className="text-[10px] text-muted-foreground">Matricule: {s.matricule}</div>
                        </TableCell>
                        <TableCell className="text-center">
                          <div className={cn(
                            "text-base font-bold",
                            (s.moyenne || 0) < 10 ? "text-destructive" : "text-foreground"
                          )}>
                            {s.moyenne?.toFixed(2) || '-'}
                          </div>
                        </TableCell>
                        <TableCell>
                          <Select 
                            ...
                            onValueChange={(val) => handleDecisionChange(s.eleve_id, val ?? '')}
                            disabled={!isClosing}
                          >
                            <SelectTrigger className={cn(
                              "h-8 text-xs font-bold",
                              isAdmitted ? "text-green-600 border-green-200" : "text-destructive border-destructive/20"
                            )}>
                              <SelectValue />
                            </SelectTrigger>
                            <SelectContent>
                        ...
                        <TableCell>
                           <Input 
                            ...
                            onChange={e => setDecisions(prev => ({ ...prev, [s.eleve_id]: { ...prev[s.eleve_id], comment: e.target.value } }))}
                            disabled={!isClosing}
                           />
                        </TableCell>
                      </TableRow>
