...
export default function GradesBulkPage() {
  ...
  const [period] = useState("TRIMESTRE_1");
  const [evalType] = useState("INTERROGATION");
  ...
  const handleImport = async () => {
    ...
    if (hasErrors) {
      toast.error(
        "Veuillez corriger les erreurs dans le fichier avant d'importer.",
      );
      return;
    }
    ...
    try {
      ...
      toast.error(
        error instanceof Error ? error.message : "Échec de l'importation.",
      );
    } finally {
      setIsSaving(false);
    }
                      ...
                      >
                        ...
                        <TableCell>
                          {row.error ? (
                            ...
                            <span className="text-[10px] font-bold uppercase text-green-600 italic">
                              OK
                            </span>
                          )}
                        </TableCell>
