  ...
  const handleDelete = async (id: number) => {
    ...
    try {
      ...
      toast.error(
        error instanceof Error
          ? error.message
          : "Impossible de supprimer cette catégorie (elle est peut-être déjà utilisée).",
      );
    }
  };
            ...
            {loading ? (
              ...
              categories.map((cat) => (
                <TableRow key={cat.id}>
                  ...
                  <TableCell>
                    {FEE_TYPES.find((t) => t.value === cat.type)?.label ||
                      cat.type}
                  </TableCell>
      ...
      <Sheet open={sheetOpen} onOpenChange={setSheetOpen}>
        <SheetContent className="w-[400px] sm:w-[540px] overflow-y-auto">
          <SheetHeader>
            ...
            <SheetDescription>
              Définissez le montant total et les éventuelles échéances de
              paiement.
            </SheetDescription>
          </SheetHeader>
                  ...
                  {fields.length === 0 ? (
                    <div className="flex flex-col items-center justify-center py-8 rounded-xl border border-dashed bg-muted/10 text-muted-foreground">
                      <p className="text-xs italic">
                        Aucun échéancier défini. Le paiement sera dû en une
                        fois.
                      </p>
                    </div>
                  ) : (
                    <div className="space-y-3">
                      {fields.map((field, index) => (
                        <div
                          key={field.id}
                          className="group relative grid grid-cols-[1fr_1fr_auto] items-end gap-3 rounded-xl border bg-card p-3 shadow-sm transition-all hover:shadow-md"
                        >
                          <FormField
                            control={form.control}
                            ...
                            render={({ field: fField }) => (
                              <FormItem className="space-y-1">
                                <FormLabel className="text-[10px] font-bold text-muted-foreground uppercase px-1">
                                  Date limite
                                </FormLabel>
                                <FormControl>
                                  <Input
                                    type="date"
                                    className="h-9 border-none bg-muted/30"
                                    {...fField}
                                  />
                                </FormControl>
                            ...
                            render={({ field: fField }) => (
                              <FormItem className="space-y-1">
                                ...
                                <FormControl>
                                  <Input
                                    type="number"
                                    className="h-9 border-none bg-muted/30 font-mono text-xs"
                                    placeholder="0"
                                    {...fField}
                                  />
                                </FormControl>
