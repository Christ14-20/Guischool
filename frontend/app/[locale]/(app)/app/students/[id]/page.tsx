...
import { CreditCard, Gavel, Star, UserPlus } from "lucide-react";
...
export default function StudentDetailPage() {
  ...
  const [attendanceView, setAttendanceView] =
    useState<AttendanceViewMode>("list");
  const [financeLoading, setFinanceLoading] = useState(true);
  const [payments, setPayments] = useState<PaymentItem[]>([]);
  const [financeSummary, setFinanceSummary] =
    useState<StudentFinancialSummary | null>(null);
  ...
  const timelineEvents = useMemo(() => {
    ...
    history?.inscriptions.forEach((ins) => {
      events.push({
        date: ins.date_inscription as string,
        type: "INSCRIPTION",
        title:
          ins.type_inscription === "REINSCRIPTION"
            ? "Réinscription"
    ...
    history?.decisions_fin_annee.forEach((dec) => {
      events.push({
        date: dec.date_decision as string,
        type: "DECISION",
    ...
    history?.notes_validees.forEach((grade) => {
      events.push({
        date: grade.created_at as string,
        type: "NOTE",
  ...
  const totalCoefficient = grades.reduce(
    (sum, grade) => sum + (grade.coefficient || 0),
    0,
  );
  const totalWeighted = grades.reduce(
    (sum, grade) => sum + (grade.weighted_score || 0),
    0,
  );
  const average = totalCoefficient > 0 ? totalWeighted / totalCoefficient : 0;

  const mention =
    average >= 16 ? "Excellent" : average >= 12 ? "Passable" : "Insuffisant";
  const mentionTone =
    average >= 16 ? "success" : average >= 12 ? "warning" : "danger";
  ...
  const attendanceRate =
    totalAttendances > 0
      ? (attendanceCounts.present / totalAttendances) * 100
      : 0;
  ...
  const attendancesByDate = [...attendances].sort((a, b) =>
                ...
                <CardHeader className="flex flex-row items-center justify-between">
                  ...
                  <div className="flex items-center gap-2">
                    <Button
                      ...
                      size="sm"
                      variant={
                        attendanceView === "list" ? "default" : "outline"
                      }
                    ...
                    <Button
                      ...
                      size="sm"
                      variant={
                        attendanceView === "calendar" ? "default" : "outline"
                      }
                ...
                <CardHeader className="flex flex-row items-center justify-between">
                  ...
                  >
                    <Button
                      onClick={() =>
                        router.push(
                          `/${locale}/app/finance/payments/new?student=${id}`,
                        )
                      }
                    >
                    ...
                    <>
                      <div className="grid gap-4 md:grid-cols-3">
                        <div className="rounded-lg border p-4">
                          <p className="text-xs text-muted-foreground">
                            Total dû (Année en cours)
                          </p>
                          <p className="text-xl font-bold">
                            {formatCurrency(financeSummary?.total_due || 0)}
                          </p>
                        </div>
                        <div className="rounded-lg border p-4">
                          <p className="text-xs text-muted-foreground">
                          ...
                          <p className="text-xl font-bold text-green-600">
                            {formatCurrency(financeSummary?.total_paid || 0)}
                          </p>
                        </div>
                        <div className="rounded-lg border p-4">
                          <p className="text-xs text-muted-foreground">
                          ...
                          <p
                            className={`text-xl font-bold ${(financeSummary?.balance || 0) > 0 ? "text-destructive" : "text-green-600"}`}
                          >
