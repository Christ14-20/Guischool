...
const NAV_ITEMS: NavItem[] = [
  ...
  },
  {
    ...
    href: "/app/support",
    icon: CircleHelp,
    children: [{ label: "Tickets", href: "/app/support/tickets" }],
  },
  {
    label: "Parametres",
    href: "/app/settings",
...
function SidebarLinks({
    ...
    () =>
      NAV_ITEMS.filter(
        ...
        children: item.children?.filter(
          (child) =>
            !child.roles ||
            (normalizedRole ? child.roles.includes(normalizedRole) : false),
        ),
      })),
    [normalizedRole],
  );
  ...
  useEffect(() => {
    // eslint-disable-next-line react-hooks/set-state-in-effect -- function updater is the correct pattern here
      ...
      {filteredItems.map((item) => {
        ...
        return (
          <div key={item.href} className="space-y-1">
            ...
            expandedSections[item.href] ? (
              <div
                className={cn("space-y-1 pl-10", compact && "hidden lg:block")}
              >
    ...
    <>
      <div className="border-b px-4 py-3 md:hidden">
        <Sheet open={isMobileOpen} onOpenChange={setMobileOpen}>
          ...
          <SheetContent
            side="left"
            className="w-80 border-none bg-primary p-0 text-primary-foreground"
          >
      ...
      <aside className="hidden border-r border-primary-foreground/18 bg-primary text-primary-foreground md:flex md:w-20 md:flex-col lg:w-72">
        <div className="border-b border-primary-foreground/16 px-3 py-4 lg:px-4">
          ...
          <p className="mt-1 truncate text-sm font-semibold md:text-center lg:text-left">
            {schoolName}
          </p>
        </div>
